    ...
    <div className="min-h-screen bg-[#FBF7F1] py-12 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-3xl border border-black/5 shadow-sm overflow-hidden">
          {loading ? (
            <p className="p-10 text-center text-[#5B5964]">{t.loading}</p>
          ) : offline ? (
            <div className="p-10">
              <OfflineCard onRetry={load} />
            </div>
          ) : history.length === 0 ? (
            <p className="p-10 text-center text-[#5B5964]">{t.empty}</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[700px]">
                <thead>
                  <tr className="text-left text-xs uppercase tracking-wider text-black/40 border-b border-black/5">
                    <th className="py-3 px-4">{t.col_file}</th>
                    <th className="py-3 px-4">{t.col_date}</th>
                    <th className="py-3 px-4">{t.col_admin}</th>
                    <th className="py-3 px-4">{t.col_imported}</th>
                    <th className="py-3 px-4">{t.col_skipped}</th>
                    <th className="py-3 px-4">{t.col_failed}</th>
                ...
                <tbody>
                  {history.map((h) => (
                    <tr key={h._id} className="border-b border-black/5 last:border-0">
                      <td className="py-3 px-4 font-medium text-[#15141A]">{h.fileName}</td>
                      <td className="py-3 px-4 text-[#5B5964]">
                        {new Date(h.createdAt).toLocaleString()}
                      </td>
                      <td className="py-3 px-4 text-[#5B5964]">
                        {h.importedBy?.fullName || "—"}
                      </td>
                      <td className="py-3 px-4 font-semibold text-[#0F6E66]">{h.imported}</td>
                      <td className="py-3 px-4 text-[#B45309]">{h.skipped}</td>
                      <td className="py-3 px-4 text-[#C81E3A]">{h.failed}</td>
                      <td className="py-3 px-4">
                        {h.undone ? (
                          ...
                          <button
                            ...
                            className="text-xs font-semibold bg-[#C81E3A] hover:bg-[#A11530] disabled:opacity-50 text-white px-3 py-1.5 rounded-lg transition"
                          >
